...
/* Video component that only renders when Clerk context is available */
function VideoWithClerk({ children }: { children: React.ReactNode }) {
...
/* Error boundary to catch Clerk context errors */
...
> {
  ...
  static getDerivedStateFromError(error: Error) {
    // Check if it's a Clerk context error
    if (
      error.message.includes(
        "useUser can only be used within the <ClerkProvider",
      )
    ) {
  ...
  componentDidCatch(error: Error, errorInfo: React.ErrorInfo) {
    if (
      ...
      console.log("Clerk context not available, video features disabled");
    }
  }
...
const VideoProvider = ({ children }: { children: React.ReactNode }) => {
  return (
    <ClerkErrorBoundary fallback={children}>
      <VideoWithClerk>{children}</VideoWithClerk>
    </ClerkErrorBoundary>
  );
};
